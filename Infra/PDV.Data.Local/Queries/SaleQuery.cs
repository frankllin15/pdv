...
public class SaleQuery : ISaleQuery
{
    ...
    public async Task<SaleDto?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        var sale = await connection.QueryFirstOrDefaultAsync<SaleQueryResult>(saleSql, new { Id = id });
        if (sale == null) return null;

        var itemResults = await connection.QueryAsync<SaleItemQueryResult>(itemsSql, new { SaleId = id });
        ...
        var paymentResults = await connection.QueryAsync<PaymentQueryResult>(paymentsSql, new { SaleId = id });
